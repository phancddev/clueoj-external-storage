...
async fn problem_readiness(
    ...
    .fetch_optional(&s.db)
    .await?;
    // Judging readiness, not snapshot verification. The judge reads the local
    // folder directly, so any folder that scans cleanly and carries a valid
    // canonical archive is usable NOW — dirty flags and pending snapshots are
    // background backup concerns that must never block grading. Only a folder
    // that cannot be scanned (corrupt/locked) stays partial.
    let usable = canonical.is_some();
    Ok(ProblemReadyResponse {
        ready: usable,
        local_status: if usable { "present".to_string() } else { "partial".to_string() },
